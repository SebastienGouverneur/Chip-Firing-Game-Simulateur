...
public class KChips {
    ...
    public KChips(int amountOfChips, int nbNode) {
        returnValue = new HashSet<>();
        Set<List<Integer>> res = accelAsc(amountOfChips, nbNode);
        for (List<Integer> original : res) {
            final Set<List<Integer>> generatedPerm = generatePerm(original);
            returnValue.add(generatedPerm);
        }
    }   
